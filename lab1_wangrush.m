...
%% Student Information
% Student Name: Rushi Wang 
% 
% Student Number: 1004927564
...
N=101; % a vector of N points between -3 and 7 (equally spaced and inclusive)
h = (7-(-3))/(N-1); %the spacing magnitude between each x point 
x = zeros(N,1);     %vector of the x points between -3 and 7

for j = 1:N %loop through each x point 
    x(j) = (j-1) * h; %setting the values of the vector of the x points between -3 and 7
end
...
for k= 1:N
    y(k)=abs(x(k))*x(k); %Loop to create vector y
end

plot(x,y) %plotting the x.x graph
...
%y(1) = 0, c2=-1                                               
%y(1) = 1, c3=e-1
%Plots for 3 particular solutions
...
xlabel('t');
ylabel('y(t)');
title('ODE of Three Particular Solutions');
legend('y1, y(1) = -1', 'y2, y(1) = 0','y3, y(1) = 1');
...
f = @(x) ((x.^3 - abs(x)) .* log((x.^2)+1)); %definition of function
%Computation of values at x=0,1,-1
x=[0,1,-1];
fprintf('f(%g) = %g\n', x, f(x));

x=linspace(-5,5,100); %Plotting the function 
plot(x,f(x))
title('Plotting f(x) Using Inline Functions');
xlabel('x');
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4; %defining the functions 
x0=0; y0=0;
c=f(x0,y0); %finding the value of c for 

f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4 - c;
...
plot(xvals, yvals);
title('f(x,y) = y + y^3 - cos x + x^2 - x^4')
xlabel('x');
ylabel('y');
%% Exercise 10
...
%Particular Solution: y-exp(-sin(x)+(x^3));
f = @(x,y) y-exp(-sin(x)+(x.^3)+c);
...
% As x approaches positive or negative infinity, y approaches 0
%The solutions are symmetric about the y-axis because y is squared, making dy/dx
%   negative if x is negative y(-x) and dy/dx positive if x is positive. Since y is always positive because it is squared, the value of the slopes 
%   is dependant on the sign of x. As a result of the slopes going from negative to right and resulting in
%   concave up solutions, solutions end up being symmetric about the
%   y-axis. 
%  
...
%Constant solutions that are approached as x goes to infinity: y=-1, 1, 2

% How y_0 determines which constant solution is approached: 
% if y_0 is between -(infinity) to -1, the solution approaches y=-1 as x
%   approaches infinity 
% if y_0 is between -1 to 1, the solution approaches y=-1 as x approaches
%   -(infinity) and approaches y=1 as x approaches infinity
% if y_0 is between 1 to 2, the solution approaches y=1 as x approaches infinity
%   and approaches y=2 as x approaches -(infinity)
% if y_0 is between 2 and infinity, the solution approaches y=infinity 
